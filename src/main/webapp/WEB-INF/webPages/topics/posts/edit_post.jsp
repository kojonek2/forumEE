...
<html class="h-100">
	<head>
		<meta charset="UTF-8">
		<title>Edit post in ${requestScope.topic.title}</title>
		...
		<div class="d-flex justify-content-center mt-3">
			<div class="jumbotron d-inline-flex flex-column col-10 col-md-6">
				...
				<c:forEach varStatus="status" var="post" items="${requestScope.topic.posts}">
					...
					<c:if test="${requestScope.postToEdit.id eq post.id}">
						<jsp:include page="/WEB-INF/fragments/topics/edit_post.jspf"/>
					</c:if>
					<c:if test="${requestScope.postToEdit.id ne post.id}">
